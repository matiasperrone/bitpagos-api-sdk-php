...
class ReflectionUtil
{
	...
	{
		if ($class == get_class( new BitPagosModel() ))
		{
			// Make it generic if BitPagosModel is used for generating this
			return get_class( new BitPagosModel() );
		}
		...
		{
			throw new BitPagosConfigurationException( "Getter function for '$propertyName' in '$class' class should have a proper return type." );
		}
	}
	...
	public static function isPropertyClassArray($class, $propertyName)
	{
		...
		$annotations = self::propertyAnnotations( $class, $propertyName );
		if ($annotations && isset( $annotations['return'] ))
		{
			$param = $annotations['return'];
		}
	...
	public static function propertyAnnotations($class, $propertyName)
	{
		...
		$annotations = & self::$propertiesType[$class][$propertyName];
		if ($annotations)
		{
			return $annotations;
		}
		
		$refl = & self::$propertiesRefl[$class][$propertyName];
		if (! $refl)
		{
	...
	{
		return method_exists( $class, "get" . ucfirst( $propertyName ) ) ? "get" . ucfirst( $propertyName ) : "get" .
										 preg_replace_callback( "/([_\-\s]?([a-z0-9]+))/", "self::replace_callback", $propertyName );
	}
}
